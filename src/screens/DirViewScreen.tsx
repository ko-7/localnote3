...
import { DirViewEditModal } from '../components/DirViewEditModal';
import { DirViewDeleteModal } from '../components/DirViewDeleteModal';
...
export const DirViewScreen: React.FC = () => {
    ...
    // フォルダ編集用モーダルの表示/非表示のステータス管理（DirViewEditModalに渡してtrue,false切り替えをさせる）
    const [ isDirViewEditModalOpened, setIsDirViewEditModalOpened ] = useState<boolean>(false);
    const updateIsDirViewEditModalOpened = async () => {     //isDirViewEditModalOpened,setIsDirViewEditModalOpened両方子コンポーネントに渡すより、isDirViewEditModalOpenedを切り替える関数つくって渡す方が良い！
        await setIsDirViewEditModalOpened(!isDirViewEditModalOpened);
    }

    // フォルダ削除用モーダルの表示/非表示のステータス管理（DirViewDeleteModalに渡してtrue,false切り替えをさせる）
    const [ isDirViewDeleteModalOpened, setIsDirViewDeleteModalOpened ] = useState<boolean>(false);
    const updateIsDirViewDeleteModalOpened = async () => {     //isDirViewDeleteModalOpened,setIsDirViewDeleteModalOpened両方子コンポーネントに渡すより、isDirViewDeleteModalOpenedを切り替える関数つくって渡す方が良い！
        await setIsDirViewDeleteModalOpened(!isDirViewDeleteModalOpened);
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            {/* ScrollViewにflexを設定すると不具合発生するためViewを置きflexを設定する */}
            <View style={{flex:8}}>
                <ScrollView>
                            ...
                            <View style={styles.dataRow}>
...

                                {
                                    ...
                                    <View style={styles.actions}>
                                        <DirViewEditModal id={item.id} updateIsDirViewEditModalOpened={updateIsDirViewEditModalOpened} /><Text> </Text>
